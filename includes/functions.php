<?php
	...
	// adding new project code begins here
	if(isset($_POST['add_project'])){
		$ProName = htmlspecialchars($_POST['ProName']);
		$ProType = $_POST['ProType'];
		$start_date = htmlspecialchars($_POST['start_date']);
		$end_date = htmlspecialchars($_POST['end_date']);
		$ProDuration = htmlspecialchars($_POST['ProDuration']);
		$ProPriority = htmlspecialchars($_POST['ProPriority']);
		$price = htmlspecialchars($_POST['price']);
		$Description = $_POST['Description'];

		$sql = "INSERT INTO projects(Pro_Name,Pro_desc,Pro_type,Pro_start,Pro_end,Pro_duration,Pro_priority,Pro_price)
			values(:ProName,:Description,:ProType,:start_date,:end_date,:ProDuration,:ProPriority,:price)";
			$query = $dbh->prepare($sql);
			$query->bindParam(':ProName',$ProName,PDO::PARAM_STR);
			$query->bindParam(':ProType',$ProType,PDO::PARAM_STR);
			$query->bindParam(':start_date',$start_date,PDO::PARAM_STR);
			$query->bindParam(':end_date',$end_date,PDO::PARAM_STR);
			$query->bindParam(':ProDuration',$ProDuration,PDO::PARAM_STR);
			$query->bindParam(':ProPriority',$ProPriority,PDO::PARAM_STR);
			$query->bindParam(':price',$price,PDO::PARAM_STR);
			$query->bindParam(':Description',$Description,PDO::PARAM_STR);
			$query->execute();
			$lastInsert = $dbh->lastInsertId();

			if(!file_exists($_POST["ProName"])) 
			{
				mkdir("Files Managment/" . $_POST["ProName"], 0777, true);
				//echo 'Folder Created';
			}

			if($lastInsert>0){
				echo "<script>alert('New Project Has Been Added');</script>";
				echo "<script>window.location.href='projects.php';</script>";
			}else{
				echo "<script>alert('Something went wrong.');</script>";
			}
	}
	//adding  project ends here
	...
	//adding Project Type code starts here
	elseif(isset($_POST['add_project_type'])){
		$project_type = $_POST['project_type'];
		$sql = "INSERT INTO project_type(project_type )VALUES(:name)";
		$query = $dbh->prepare($sql);
		$query->bindParam(':name',$project_type,pdo::PARAM_STR);
		$query->execute();
		$lastInsert = $dbh->lastInsertId();
		if($lastInsert>0){
			echo "<script>alert('Project Type Has Been Added');</script>";
			echo "<script>window.location.href='project_type.php';</script>";
		}else{
			echo "<script>alert('Something went wrong.');</script>";
		}
	}//adding Project Type code ends here
